...
__global__ void collisionKernel(Float *pSrc /**[9][NX+2][NY+2]**/, Float *pDst /**[9][NX+2][NY+2]**/, Float *pDensity /**[NX+2][NY+2]**/, Vec2 *pVelocity /**[NX+2][NY+2]**/) {
    ...
    uint32_t xy = x*NY_SIZE + y;
    uint32_t tx = threadIdx.y + 1;
    uint32_t ty = threadIdx.x + 1;

    __shared__ Float density[BLOCK_SIZE+2][BLOCK_SIZE+2];
    __shared__ Vec2  velocity[BLOCK_SIZE+2][BLOCK_SIZE+2];
    __shared__ Float df[BLOCK_SIZE+2][BLOCK_SIZE+2][Q];

    density[tx][ty]  = pDensity[xy];
    velocity[tx][ty] = pVelocity[xy];

    for(int32_t q = 0; q < Q; ++q) {
        df[tx][ty][q] = pSrc[q*NX_SIZE*NY_SIZE + xy];
    }

    // bottom
    if(ty == 1) {
        density[tx][0]  = pDensity[xy - 1];
        velocity[tx][0] = pVelocity[xy - 1];

        for(int32_t q = 0; q < Q; ++q) {
            df[tx][0][q] = pSrc[q*NX_SIZE*NY_SIZE + xy - 1];
        }
    }

    // top
    if(ty == BLOCK_SIZE) {
        density[tx][BLOCK_SIZE+1]  = pDensity[xy + 1];
        velocity[tx][BLOCK_SIZE+1] = pVelocity[xy + 1];

        for(int32_t q = 0; q < Q; ++q) {
            df[tx][BLOCK_SIZE + 1][q] = pSrc[q*NX_SIZE*NY_SIZE + xy + 1];
        }
    }

    // left
    if(tx == 1) {
        density[0][ty]  = pDensity[xy - NY_SIZE];
        velocity[0][ty] = pVelocity[xy - NY_SIZE];

        for(int32_t q = 0; q < Q; ++q) {
            df[0][ty][q] = pSrc[q*NX_SIZE*NY_SIZE + xy - NY_SIZE];
        }
    }

    // right
    if(tx == BLOCK_SIZE) {
        density[BLOCK_SIZE+1][ty]  = pDensity[xy + NY_SIZE];
        velocity[BLOCK_SIZE+1][ty] = pVelocity[xy + NY_SIZE];
        for(int32_t q = 0; q < Q; ++q) {
            df[BLOCK_SIZE + 1][ty][q] = pSrc[q*NX_SIZE*NY_SIZE + xy + NY_SIZE];
        }
    }

    // bottom left
    if(ty == 1 and  tx == 1) {
        density[0][0]  = pDensity[xy - NY_SIZE - 1];
        velocity[0][0] = pVelocity[xy - NY_SIZE - 1];
        for(int32_t q = 0; q < Q; ++q) {
            df[0][0][q] = pSrc[q*NX_SIZE*NY_SIZE + xy - NY_SIZE - 1];
        }
    }

    // bottom right
    if(ty == 1 and tx == BLOCK_SIZE) {
        density[BLOCK_SIZE+1][0]  = pDensity[xy + NY_SIZE - 1];
        velocity[BLOCK_SIZE+1][0] = pVelocity[xy + NY_SIZE - 1];
        for(int32_t q = 0; q < Q; ++q) {
            df[BLOCK_SIZE + 1][0][q] = pSrc[q*NX_SIZE*NY_SIZE + xy + NY_SIZE - 1];
        }
    }

    // top left
    if(ty == BLOCK_SIZE and tx == 1) {
        density[0][BLOCK_SIZE+1]  = pDensity[xy - NY_SIZE + 1];
        velocity[0][BLOCK_SIZE+1] = pVelocity[xy - NY_SIZE + 1];
        for(int32_t q = 0; q < Q; ++q) {
            df[0][BLOCK_SIZE + 1][q] = pSrc[q*NX_SIZE*NY_SIZE + xy - NY_SIZE + 1];
        }
    }

    // top right
    if(ty == BLOCK_SIZE and tx == BLOCK_SIZE) {
        density[BLOCK_SIZE+1][BLOCK_SIZE+1]  = pDensity[xy + NY_SIZE + 1];
        velocity[BLOCK_SIZE+1][BLOCK_SIZE+1] = pVelocity[xy + NY_SIZE + 1];
        for(int32_t q = 0; q < Q; ++q) {
            df[BLOCK_SIZE + 1][BLOCK_SIZE + 1][q] = pSrc[q*NX_SIZE*NY_SIZE + xy + NY_SIZE + 1];
        }
    }
    ...
    for(int32_t q = 0; q < Q; ++q) {
        ...
        Float f                      = df[nx][ny][q];
        pDst[q*NX_SIZE*NY_SIZE + xy] = f + (feq(q, density[nx][ny], velocity[nx][ny]) - f)/tau_;
    }
}
...
void save(Vec2 velocity[NX_SIZE][NY_SIZE]) {
    ...
    sprintf(fileName, "cuda_sm_lbm_lid_cavity_%dx%d.pydat", NX, NY);
